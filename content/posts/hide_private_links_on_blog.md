...
I publish a subset of my notes which I manually mark with a `public` file tag. Sometimes published notes have links to private, unpublished, notes and by default these would all be (1) Hugo errors or (2), if we [bypass](https://gohugo.io/configuration/all/#reflinkserrorlevel) that check, a `404`. I'd like these links to not be links and instead be plain text.
...
We got something! The hex converts to \`...HUGOSHORTCODE2s0...\` (I've omitted the "HAHA" prefix and "HBHB" suffix to prevent Hugo from attempting to erroring out when it attempts to evaluate this shortcode) which corresponds to a [shortcode](https://github.com/gohugoio/hugo/blob/0de8f8607bff16308fbb28816c6e2571c7846d2d/hugolib/shortcode.go#L193) which is later [expanded](https://github.com/gohugoio/hugo/blob/0de8f8607bff16308fbb28816c6e2571c7846d2d/hugolib/shortcode.go#L737) into a link.
